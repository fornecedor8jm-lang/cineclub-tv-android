...
const navItems = ["Início", "Séries", "Terror", "Filmes", "Nuvem", "Minha lista", "Sobre"];
const DEFAULT_M3U_URL = "https://iptv-org.github.io/iptv/languages/por.m3u";
const M3U_SOURCES_KEY = "cineclub-m3u-sources";
type M3USource = { id: string; name: string; url: string; builtIn?: boolean };
...
export default function HomeScreen() {
  ...
  const persistM3uSources = async (sources: M3USource[]) => {
    const serialized = JSON.stringify(sources);
    if (Platform.OS === "web") localStorage.setItem(M3U_SOURCES_KEY, serialized);
    else await SecureStore.setItemAsync(M3U_SOURCES_KEY, serialized);
  };

  const loadM3u = async (source: string, sourceId = activeM3uSourceId) => {
    ...
    try {
      ...
      if (!text.trim().startsWith("#EXTM3U") && !text.includes("#EXTINF")) throw new Error("Formato inválido");
      const parsed = parseM3U(text);
      if (parsed.length === 0) throw new Error("Lista vazia");
      setM3uGroups(groupM3U(parsed));
      setActiveM3uSourceId(sourceId);
      setM3uStatus(`${parsed.length} itens em ${groupM3U(parsed).length} grupos`);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Erro desconhecido";
      setM3uStatus(`Falha na playlist: ${message}. A lista anterior foi mantida.`);
    }
  };

  const addM3uSource = async () => {
    const url = m3uUrl.trim();
    if (!url) return setM3uStatus("Informe uma URL M3U.");
    const customSources = m3uSources.filter((source) => !source.builtIn);
    if (customSources.length >= 3) return setM3uStatus("Limite atingido: você pode adicionar até 3 M3Us personalizados.");
    const source: M3USource = { id: `custom-${Date.now()}`, name: m3uName.trim() || `Minha lista ${customSources.length + 1}`, url };
    const next = [...m3uSources, source];
    setM3uSources(next);
    setM3uName("");
    setActiveM3uSourceId(source.id);
    await persistM3uSources(next);
    await loadM3u(source.url, source.id);
  };

  const removeM3uSource = async (sourceId: string) => {
    const source = m3uSources.find((entry) => entry.id === sourceId);
    if (!source || source.builtIn) return;
    const next = m3uSources.filter((entry) => entry.id !== sourceId);
    setM3uSources(next);
    await persistM3uSources(next);
    const fallback = next[0];
    if (fallback) {
      setM3uUrl(fallback.url);
      await loadM3u(fallback.url, fallback.id);
    }
  };
  const [browserUrl, setBrowserUrl] = useState<string | null>(null);
  const [m3uUrl, setM3uUrl] = useState(DEFAULT_M3U_URL);
  const [m3uName, setM3uName] = useState("");
  const [m3uSources, setM3uSources] = useState<M3USource[]>([{ id: "iptv-org-por", name: "Canais em português", url: DEFAULT_M3U_URL, builtIn: true }]);
  const [activeM3uSourceId, setActiveM3uSourceId] = useState("iptv-org-por");
  ...
  useEffect(() => {
    const onBack = () => {
      if (browserUrl) { setBrowserUrl(null); setBrowserTitle(""); return true; }
  ...
  useEffect(() => {
    ...
    (async () => {
      const stored = Platform.OS === "web" ? localStorage.getItem(M3U_SOURCES_KEY) : await SecureStore.getItemAsync(M3U_SOURCES_KEY);
      let sources: M3USource[] = [{ id: "iptv-org-por", name: "Canais em português", url: DEFAULT_M3U_URL, builtIn: true }];
      if (stored) {
        try {
          const parsed = JSON.parse(stored) as M3USource[];
          if (Array.isArray(parsed)) sources = [sources[0], ...parsed.filter((source) => source.id !== sources[0].id && typeof source.url === "string").slice(0, 3)];
        } catch { /* usa a fonte padrão */ }
      }
      setM3uSources(sources);
      const initial = sources[0];
      setM3uUrl(initial.url);
      void loadM3u(initial.url, initial.id);
    })();
  ...
  const openSource = (url: string, title = selected?.name ?? "Título") => {
  ...
  const handleWatch = (item: Title) => {
    ...
    if (sources.length === 1) {
      openSource(sources[0].url, item.name);
    } else {
      setSelected(item);
    }
  };
      ...
      <ScrollView style={styles.root} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} importantForAccessibility={selected || browserUrl || m3uPlayerItem ? "no-hide-descendants" : "auto"} accessible={!selected && !browserUrl && !m3uPlayerItem}>
        <View style={styles.header}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.navScroll} contentContainerStyle={styles.nav}>
            {navItems.map((item) => <Pressable key={item} focusable hasTVPreferredFocus={item === "Início"} onFocus={() => setFocusedElement(`nav-${item}`)} onPress={() => setActiveNav(item)} style={({ pressed }) => [styles.navItem, activeNav === item && styles.navActive, focusedElement === `nav-${item}` && styles.navFocused, pressed && styles.pressed]}><Text style={[styles.navText, activeNav === item && styles.navTextActive]}>{item}</Text></Pressable>)}
          </ScrollView>
        ...
        {activeNav === "Nuvem" ? <View style={styles.cloudPanel}>
          ...
          <Text style={styles.cloudText}>A fonte pública em português já vem configurada. Você pode adicionar até 3 playlists M3U personalizadas; cada uma fica salva separadamente neste dispositivo.</Text>
          <View style={styles.sourceTabs}>{m3uSources.map((source) => <View key={source.id} style={styles.sourceTabWrap}><Pressable focusable onFocus={() => setFocusedElement(`source-${source.id}`)} onPress={() => { setActiveM3uSourceId(source.id); setM3uUrl(source.url); void loadM3u(source.url, source.id); }} style={({ pressed }) => [styles.sourceTab, activeM3uSourceId === source.id && styles.sourceTabActive, focusedElement === `source-${source.id}` && styles.navFocused, pressed && styles.pressed]}><Text numberOfLines={1} style={styles.sourceTabText}>{source.name}</Text></Pressable>{!source.builtIn && <Pressable focusable onPress={() => void removeM3uSource(source.id)} style={styles.sourceRemove}><Text style={styles.sourceRemoveText}>×</Text></Pressable>}</View>)}</View>
          <TextInput value={m3uName} onChangeText={setM3uName} placeholder="Nome da nova lista (opcional)" placeholderTextColor="#7E9692" style={styles.cloudInput} />
          ...
          <View style={styles.cloudActions}>
            <FocusButton label="＋  Adicionar M3U" primary onPress={addM3uSource} />
            <FocusButton label="↻  Atualizar atual" onPress={() => loadM3u(m3uUrl, activeM3uSourceId)} />
            <FocusButton label="📱  Configurar pelo celular" onPress={startPairing} />
            <FocusButton label="▶  Testar demonstração" onPress={() => { const parsed = parseM3U(DEMO_M3U); setM3uGroups(groupM3U(parsed)); setM3uStatus(`${parsed.length} itens de demonstração`); }} />
          </View>
          {pairingToken && <View style={styles.pairingCard}><View style={styles.pairingCopy}><Text style={styles.pairingTitle}>Escaneie com o celular</Text><Text style={styles.cloudText}>O QR Code contém somente um token temporário. Não compartilhe este código.</Text><Text style={styles.pairingToken}>{pairingToken}</Text><Text style={styles.cloudStatus}>{pairingStatus.data?.state === "waiting" ? "Aguardando envio…" : "Sessão ativa por alguns minutos"}</Text></View><View style={styles.pairingQr}><PairingQr value={pairingUrl} /></View></View>}
          <Text style={styles.cloudStatus}>{m3uStatus}</Text>
          {m3uGroups.map((group, groupIndex) => <View key={group.group} style={styles.cloudGroup}><Text style={styles.rowTitle}>{group.group}</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cloudItems}>{group.items.map((item, itemIndex) => <Pressable key={item.id} focusable hasTVPreferredFocus={activeNav === "Nuvem" && groupIndex === 0 && itemIndex === 0} onFocus={() => setFocusedElement(`m3u-${item.id}`)} onPress={() => setM3uPlayerItem(item)} style={({ pressed }) => [styles.cloudItem, focusedElement === `m3u-${item.id}` && styles.navFocused, pressed && styles.pressed]}>{item.logo ? <Image source={{ uri: item.logo }} style={styles.cloudItemLogo} resizeMode="contain" /> : <View style={styles.cloudItemLogoFallback}><Text style={styles.cloudItemLogoText}>{item.name.slice(0, 1).toUpperCase()}</Text></View>}<Text numberOfLines={2} style={styles.cloudItemText}>{item.name}</Text><Text style={styles.cloudItemKind}>{item.kind === "live" ? "CANAL AO VIVO" : item.kind.toUpperCase()}</Text></Pressable>)}</ScrollView></View>)}
        </View> : <>
      ...
      <Modal visible={Boolean(browserUrl)} animationType="fade" presentationStyle="fullScreen" statusBarTranslucent onRequestClose={() => { setBrowserUrl(null); setBrowserTitle(""); }}>
        {browserUrl && <View style={styles.browserBackdrop}>
          <View style={styles.browserHeader}>
            ...
            <Pressable focusable onFocus={() => setFocusedElement("browser-close")} onPress={() => { setBrowserUrl(null); setBrowserTitle(""); }} style={({ pressed }) => [styles.browserClose, focusedElement === "browser-close" && styles.navFocused, pressed && styles.pressed]}>
